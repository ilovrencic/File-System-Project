...
use cplfs_api::types::{Buffer, FType, InodeLike, SuperBlock, Block};
...
fn proba(){
    ...
    let mut vector1  = vec![0; 10].into_boxed_slice();
    let mut vector2 = vec![0; 20].into_boxed_slice();
    let block1: Block = utils::n_block(2,100,5);
    let block2: Block = utils::n_block(3,100,9);

    block1.read_data(&mut vector1, 0);
    block2.read_data(&mut vector2,0);

    let mut vec1 = vector1.to_vec();
    let mut vec2 = vector2.to_vec();
